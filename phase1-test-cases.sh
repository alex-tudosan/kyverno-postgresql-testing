...
run_custom_test "13" "Database Connection" \
    "kubectl -n reports-server logs -l app=reports-server --tail=50 | grep -q 'database.*connected\|postgres.*connected'" \
...
run_custom_test "19" "System Recovery" \
    "kubectl delete pod -n reports-server \$(kubectl get pods -n reports-server -l app=reports-server -o jsonpath='{.items[0].metadata.name}') && sleep 30 && kubectl get pods -n reports-server -l app=reports-server --no-headers | grep -q 'Running'" \
...
echo "  - Reports Server logs: kubectl -n reports-server logs -l app=reports-server"
echo "  - Kyverno logs: kubectl -n kyverno-system logs -l app=kyverno"
