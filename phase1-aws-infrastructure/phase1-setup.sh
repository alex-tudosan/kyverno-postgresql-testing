...
# Function to create EKS cluster via Terraform
create_eks_cluster_terraform() {
    ...
    sed -i.bak "s/default = \"[^\"]*\"/default = \"$cluster_name\"/" variables.tf
    
    # Initialize Terraform
    log_step "TERRAFORM" "Initializing Terraform..."
    ...
    log_step "TERRAFORM" "Planning Terraform changes..."
    ...
    log_step "TERRAFORM" "Applying Terraform changes (this may take 15-20 minutes)..."
...
show_progress() {
    ...
    # echo "DEBUG: show_progress called with current=$current, total=$total" >&2
    
    # Handle edge cases to prevent division by zero and invalid arithmetic
    if [[ "$total" -eq 0 ]] || [[ "$current" -eq 0 ]]; then
        printf "\r${BLUE}[PROGRESS]${NC} [${current}/${total}] Waiting..."
        return
    fi
    
    # Ensure both values are valid integers
    if ! [[ "$current" =~ ^[0-9]+$ ]] || ! [[ "$total" =~ ^[0-9]+$ ]]; then
        printf "\r${BLUE}[PROGRESS]${NC} [${current}/${total}] Invalid values..."
        return
    fi
...
check_terraform_config() {
    ...
    log_success "Terraform configuration verified"
}
...
check_existing_resources() {
    ...
    # Check for existing EKS cluster using AWS CLI
    if aws eks describe-cluster --name $CLUSTER_NAME --region $AWS_REGION --profile $AWS_PROFILE &>/dev/null; then
        ...
        log_step "EKS" "EKS cluster '$CLUSTER_NAME' not found - will create via Terraform"
        # Note: We'll create the cluster later in the setup process
    fi
...
wait_for_eks_nodes() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        if node_count=$(kubectl get nodes --no-headers 2>/dev/null | wc -l | tr -d ' '); then
            # Handle case where no nodes exist yet (early in deployment)
...
cleanup_on_failure() {
    ...
    if aws rds describe-db-subnet-groups --db-subnet-group-name reports-server-subnet-group --profile $AWS_PROFILE &>/dev/null; then
...
log_step "CONFIG" "Validating configuration variables..."
if [[ -z "${CLUSTER_NAME:-}" ]]; then
    log_error "CLUSTER_NAME is not set! Configuration loading failed."
    exit 1
fi
if [[ -z "${AWS_REGION:-}" ]]; then
    log_error "AWS_REGION is not set! Configuration loading failed."
    exit 1
fi
if [[ -z "${AWS_PROFILE:-}" ]]; then
    log_error "AWS_PROFILE is not set! Configuration loading failed."
    exit 1
fi
log_success "Configuration validation passed: CLUSTER_NAME=$CLUSTER_NAME, AWS_REGION=$AWS_REGION, AWS_PROFILE=$AWS_PROFILE"
...
# EKS cluster setup - either use existing or create via Terraform
log_step "EKS" "Setting up EKS cluster: $CLUSTER_NAME"
log_step "DEBUG" "Using CLUSTER_NAME: '$CLUSTER_NAME' from configuration"

# Check if cluster exists, if not create it via Terraform
if ! aws eks describe-cluster --name $CLUSTER_NAME --region $AWS_REGION --profile $AWS_PROFILE &>/dev/null; then
    log_step "EKS" "EKS cluster '$CLUSTER_NAME' not found - creating via Terraform..."
    
    # Create EKS cluster via Terraform
    if ! create_eks_cluster_terraform "$CLUSTER_NAME" "$AWS_REGION" "$AWS_PROFILE"; then
        log_error "Failed to create EKS cluster via Terraform"
        exit 1
    fi
...
# Create RDS subnet group (only if not using existing RDS)
if [[ "${SKIP_RDS_CREATION:-false}" != "true" ]]; then
...
log_step "RDS" "Checking if RDS instance '$RDS_INSTANCE_ID' already exists..."
if aws rds describe-db-instances --db-instance-identifier "$RDS_INSTANCE_ID" --region $AWS_REGION --profile $AWS_PROFILE &>/dev/null; then
    log_warning "RDS instance '$RDS_INSTANCE_ID' already exists - will use existing instance"
    
    # Get the existing instance status and endpoint
    ...
    log_step "RDS" "Existing instance status: $RDS_STATUS, Endpoint: $RDS_ENDPOINT"
    
    # Check if instance is available
    if [[ "$RDS_STATUS" == "available" ]]; then
        log_success "Using existing RDS instance '$RDS_INSTANCE_ID'"
        SKIP_RDS_CREATION=true
    else
        log_warning "RDS instance exists but status is '$RDS_STATUS' - waiting for it to become available"
        SKIP_RDS_CREATION=true
    fi
else
    log_step "RDS" "RDS instance '$RDS_INSTANCE_ID' not found - will create new one"
    SKIP_RDS_CREATION=false
fi
...
# Handle RDS instance creation or reuse
if [[ "${SKIP_RDS_CREATION:-false}" == "true" ]]; then
    log_step "RDS" "Using existing RDS instance: $RDS_INSTANCE_ID"
    log_success "RDS endpoint: $RDS_ENDPOINT"
    
    # For existing instances, we need to get the password from the instance
    # Since we can't retrieve the password, we'll need to reset it
    log_step "RDS" "Resetting password for existing RDS instance..."
    ...
    if aws rds modify-db-instance \
        ...
        log_step "RDS" "Waiting for password update to complete..."
        sleep 60
    else
        log_warning "Could not reset password, will try to connect with generated password"
...
log_step "MONITORING" "Checking monitoring stack status..."
if helm list -n monitoring | grep -q "monitoring"; then
    log_warning "Monitoring stack already exists - skipping installation"
    log_step "MONITORING" "Verifying monitoring stack is ready..."
    if ! wait_for_helm_release "monitoring" "monitoring" 5; then
        log_error "Existing monitoring stack is not ready"
        exit 1
    fi
    log_success "Using existing monitoring stack"
else
...
# Check if Kyverno already exists
log_step "KYVERNO" "Checking Kyverno status..."
if helm list -n kyverno | grep -q "kyverno"; then
    log_warning "Kyverno already exists - skipping installation"
    log_step "KYVERNO" "Verifying Kyverno is ready..."
    if ! wait_for_helm_release "kyverno" "kyverno" 5; then
        log_error "Existing Kyverno is not ready"
        exit 1
    fi
    log_success "Using existing Kyverno"
else
...
# Wait for Kyverno to be ready (only if we just installed it)
if ! helm list -n kyverno | grep -q "kyverno"; then
...
# Check if Reports Server already exists
log_step "REPORTS-SERVER" "Checking Reports Server status..."
if helm list -n kyverno | grep -q "reports-server-db"; then
    log_warning "Reports Server already exists - skipping installation"
    log_step "REPORTS-SERVER" "Verifying Reports Server is ready..."
    if ! wait_for_helm_release "kyverno" "reports-server-db" 5; then
        log_error "Existing Reports Server is not ready"
        exit 1
    fi
    log_success "Using existing Reports Server"
else
...
# Wait for Reports Server to be ready (only if we just installed it)
if ! helm list -n kyverno | grep -q "reports-server-db"; then
